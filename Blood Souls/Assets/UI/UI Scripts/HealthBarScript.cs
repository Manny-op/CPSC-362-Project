...
public class HealthBarScript : MonoBehaviour
{
    ...
    public IEnumerator StaminaTickDown(int amt)
    {
        yield return new WaitForSeconds(1);

        while(SEffectImage.fillAmount > staminaBar.value)
        {
            SEffectImage.fillAmount -= useSpeed;
            yield return regenTick;
        }
    }
    ...
    public void StaminaUse(int amt)
    {
        ...
        {
            player.playerStamina = Mathf.Max(player.playerStamina - amt, 0);

            if(tickDown!= null) { StopCoroutine(tickDown); }

            StartCoroutine(StaminaTickDown(amt));
